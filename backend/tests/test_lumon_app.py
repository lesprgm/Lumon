...
def test_parse_args_defaults_to_plugin_first_flow(monkeypatch) -> None:
    ...
    assert args.frontend_origin == "http://127.0.0.1:8000"
...
def test_run_keeps_backend_alive_if_frontend_exits(monkeypatch, capsys) -> None:
    ...
    assert "Lumon frontend exited; backend will keep running for browser commands." not in capsys.readouterr().err
    assert terminated
