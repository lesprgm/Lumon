...
def create_app() -> FastAPI:
    ...
    async def healthz() -> dict[str, str]:
        return {"status": "ok", "protocol_version": PROTOCOL_VERSION, "runtime_version": RUNTIME_VERSION}
...
""".strip()
    ...
    async def local_observe_opencode(
        ...
        attached = await manager.attach_local_opencode_observer(
            payload,
            frontend_origin=payload.frontend_origin or f"{request.url.scheme}://{request.url.hostname}:5173",
        )
    ...
    async def local_session_approve(session_id: str, payload: CheckpointPayload, request: Request) -> dict:
        _require_local_client(request)
        try:
            return await manager.resolve_local_checkpoint(session_id, payload.checkpoint_id, approve=True)
        except KeyError as exc:
            raise HTTPException(status_code=404, detail="Session not found") from exc

    @app.post("/api/local/session/{session_id}/reject")
    async def local_session_reject(session_id: str, payload: CheckpointPayload, request: Request) -> dict:
        _require_local_client(request)
        try:
            return await manager.resolve_local_checkpoint(session_id, payload.checkpoint_id, approve=False)
        except KeyError as exc:
            raise HTTPException(status_code=404, detail="Session not found") from exc
    ...
    async def session_artifact(session_id: str, request: Request) -> dict:
        ...
        session_dir = Path(__file__).resolve().parents[2] / "output" / "sessions" / session_id
    ...
    async def session_keyframe(session_id: str, filename: str, request: Request) -> FileResponse:
        _require_local_client(request)
        keyframe_path = Path(__file__).resolve().parents[2] / "output" / "sessions" / session_id / "keyframes" / filename
