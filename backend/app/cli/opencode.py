...
from .common import BACKEND_ROOT, DEFAULT_BACKEND_ORIGIN, DEFAULT_FRONTEND_ORIGIN
from .session_bootstrap import bootstrap_session, build_ws_url

if str(BACKEND_ROOT) not in sys.path:
    sys.path.insert(0, str(BACKEND_ROOT))

from app.opencode_observer import (
    DEFAULT_OPENCODE_DB_PATH,
    OpenCodeSQLiteObserver,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--backend-origin", default=DEFAULT_BACKEND_ORIGIN, help="Lumon backend origin.")
    parser.add_argument("--frontend-origin", default=DEFAULT_FRONTEND_ORIGIN, help="Lumon frontend origin.")
    parser.add_argument(
        "--web-mode",
    ...
    parser.add_argument("--web-bridge", choices=("playwright_native", "off"), default=None, help=argparse.SUPPRESS)
    parser.add_argument("--db-path", default=str(DEFAULT_OPENCODE_DB_PATH), help="OpenCode sqlite database path.")
    parser.add_argument(
        "--auto-delegate",
        action="store_true",
        help="Automatically launch the delegated browser bridge when browser-capable OpenCode work is detected.",
    )
    parser.add_argument("--session-timeout", type=float, default=20.0, help="Seconds to wait for a new OpenCode session to appear in sqlite.")
    parser.add_argument("--discover-poll-interval", type=float, default=0.08, help="Seconds between sqlite polls while discovering the active OpenCode session.")
    parser.add_argument("--idle-poll-min", type=float, default=0.1, help="Minimum seconds between sqlite polls after the OpenCode session is locked.")
    parser.add_argument("--idle-poll-max", type=float, default=0.6, help="Maximum seconds between sqlite polls after the OpenCode session is locked.")
    parser.add_argument("--observed-session-id", default=None, help="Existing OpenCode session id to follow instead of discovering a new one.")
    parser.add_argument("opencode_args", nargs=argparse.REMAINDER, help="Arguments forwarded to `opencode` after `--`.")
...
def forwarded_opencode_args(raw_args: list[str]) -> list[str]:
    ...
    if args and args[0] in {"run", "serve", "attach", "session", "export"}:
        raise SystemExit("lumon opencode only wraps interactive `opencode [project]` sessions, not opencode subcommands.")
    return args
...
async def run() -> int:
    ...
    print("[lumon opencode] Internal debug wrapper. Primary user flow is plain: opencode .", flush=True)
